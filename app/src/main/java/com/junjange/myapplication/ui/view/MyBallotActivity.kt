...
class MyBallotActivity  : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private val binding by lazy { ActivityMyBallotBinding.inflate(layoutInflater) }
    private val viewModel by lazy { ViewModelProvider(this, MyBallotViewModel.Factory(application))[MyBallotViewModel::class.java] }
    private lateinit var retrofitAdapter: MyBallotAdapter
    ...
    private fun setView(context : Context){
        retrofitAdapter =  MyBallotAdapter(context).apply {
    ...
    private fun setObserver() {
        viewModel.retrofitMyBallot.observe(this, {

            viewModel.retrofitMyBallot.value?.let { it1 -> retrofitAdapter.setData(it1) }
        })
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        ...
        when(item.itemId){
            ...
            R.id.allPollsDrawer-> {
                startActivity( Intent(this@MyBallotActivity, PollsActivity::class.java))

            }
            R.id.hotPollsDrawer-> {
                startActivity( Intent(this@MyBallotActivity, HotPollsActivity::class.java))

            }
            R.id.searchDrawer-> {
                startActivity( Intent(this@MyBallotActivity, SearchActivity::class.java))

            }
            R.id.myPageDrawer-> {
                // My Page 이동
                startActivity( Intent(this@MyBallotActivity, MyPageActivity::class.java))

            }

            R.id.newPollDrawer-> {
                // My Page 이동
                startActivity(Intent(this@MyBallotActivity, NewPollActivity::class.java))
